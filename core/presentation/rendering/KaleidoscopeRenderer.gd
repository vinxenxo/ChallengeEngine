...
## C6-F0.8-D6 — Kaleidoscope Renderer (Passive)
## Binds Logical Layer State directly to GPU Shader parameters.

const KALEIDOSCOPE_SHADER = preload("res://core/presentation/rendering/shaders/kaleidoscope.gdshader")
...
func _init() -> void:
	...
	_material.shader = KALEIDOSCOPE_SHADER
...
func apply_state(model: Dictionary) -> void:
	...
	_material.set_shader_parameter("symmetry_folds", float(params.get("symmetry_folds", 6.0)))
	_material.set_shader_parameter("zoom", float(params.get("zoom", 1.0)))
	
	if layer.get("color_palette", "") == "ruby":
		_material.set_shader_parameter("base_color", Color(0.9, 0.1, 0.3, 1.0))
	else:
		_material.set_shader_parameter("base_color", Color(0.9, 0.2, 0.2, 1.0))