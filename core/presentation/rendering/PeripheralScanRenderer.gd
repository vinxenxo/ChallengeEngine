...
## Binds Logical Drill State directly to GPU Shader parameters.

const PERIPHERAL_SHADER = preload("res://core/presentation/rendering/shaders/peripheral_scan.gdshader")
...
func _init() -> void:
	...
	_material.shader = PERIPHERAL_SHADER
...
func apply_state(model: Dictionary) -> void:
	...
	var stim_x := float(stimulus.get("x", 0.0)) / 100.0
	var stim_y := float(stimulus.get("y", 0.0)) / 100.0
	
	_material.set_shader_parameter("progress", progress)
	_material.set_shader_parameter("stimulus_pos", Vector2(stim_x, stim_y))
	_material.set_shader_parameter("pattern_variant", float(params.get("pattern_variant", 0.0)))
	_material.set_shader_parameter("amplitude_variant", float(params.get("amplitude_variant", 0.5)))
	_material.set_shader_parameter("base_color", Color(0.2, 0.8, 0.6, 1.0))