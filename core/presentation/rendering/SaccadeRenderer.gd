...
## Binds Logical Drill State directly to GPU Shader parameters for Stream 2013.

const SACCADE_SHADER = preload("res://core/presentation/rendering/shaders/saccade.gdshader")
...
func _init() -> void:
	...
	_material.shader = SACCADE_SHADER
...
func apply_state(model: Dictionary) -> void:
	...
	_material.set_shader_parameter("saccade_variant", float(params.get("saccade_variant", 0.0)))
	_material.set_shader_parameter("base_color", Color(0.8, 0.2, 0.9, 1.0))