...
class MainWindow(QMainWindow):
 def __init__(self,ctx,cfg,app_version="0.2.1"):
  ...
  self.introspector=BackendIntrospector(ctx);catalog=self.introspector.scan();self.families=FamilyRegistry(ctx,self.introspector).load();self.commands=CommandBuilder(ctx,catalog);self.seeds=SeedManager(ctx.paths.gui_workspace/"seeds");self.jobs=JobManager(ctx.paths.gui_logs);self.registry=ArtifactRegistry(ctx);self.products_catalog=ProductCatalog(ctx);self.snapshots=SnapshotService(ctx.paths.gui_snapshots)
 ...
 def _build(self):
  ...
  self.dashboard=DashboardPage(self.ctx,self.tools);self.review=ReviewPage(self.families);self.production=ProductionPage(self.families);self.art=ArtDirectionPage(self.ctx,self.families,self.snapshots);self.families_page=FamiliesPage(self.families);self.seeds_page=SeedsPage(self.seeds);self.products=ProductsPage(self.products_catalog,self.cfg);self.artifacts=ArtifactsPage(self.registry);self.jobs=JobsPage(self.jobs);self.logs=LogsPage(self.jobs);self.validation=ValidationPage();self.games=GamesPage(self.ctx);self.repro=ReproductionPage(self.ctx,self.families,self.commands);self.settings=SettingsPage(self.ctx,self.cfg);self.about=AboutPage(self.ctx,self.app_version)
 ...
 def _sync_running(self):
  running=self.jobs.process is not None;self.review.set_running(running);self.statusBar().showMessage("RUNNING" if running else "Ready")
 ...
 def _start(self,jtype,spec,seeds=None):
  try:j=self.jobs.start(jtype,spec,seed=(seeds[0] if seeds and len(seeds)==1 else None),batch_id="batch_"+datetime.now().strftime("%Y%m%d_%H%M%S"))
 ...
 def _finished(self,jid,code):
  self.products.refresh();self.artifacts.refresh();self._refresh_counts();self._sync_running();self.statusBar().showMessage(f"{self.jobs.jobs[jid].job_type}: {self.jobs.jobs[jid].state.value}",8000)
