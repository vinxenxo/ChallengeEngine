...
from PySide6.QtWidgets import (QWidget,QVBoxLayout,QHBoxLayout,QGridLayout,QLabel,QPushButton,QCheckBox,QSpinBox,QComboBox,QLineEdit,QPlainTextEdit,QFrame,QGroupBox,QFormLayout,QStackedWidget,QMessageBox)
...
class GeneratePage(QWidget):
    ...
    def __init__(self, families, parent=None):
        ...
        root=QVBoxLayout(self); root.setContentsMargins(30,26,30,24); root.setSpacing(16)
        title=QLabel("Generar"); title.setObjectName("PageTitle"); root.addWidget(title)
        sub=QLabel("Panel de control de generación. Una pantalla para crear vídeos, reviews y productos finales."); sub.setObjectName("PageSubtitle"); root.addWidget(sub)

        stats=QFrame(); stats.setObjectName("GenerateSummary"); sv=QHBoxLayout(stats); sv.setContentsMargins(15,12,15,12); sv.setSpacing(22)
        for label,value,meta in (("FAMILIAS",str(len(families)),"descubiertas"),("DELIVERY","720×1280","9:16 · 30 FPS"),("BASELINE","18.00 s","540 frames"),("AUDIO","ON","por defecto")):
            box=QVBoxLayout(); a=QLabel(label); a.setObjectName("CardLabel"); v=QLabel(value); v.setObjectName("GenerateStatValue"); m=QLabel(meta); m.setObjectName("CardMeta"); box.addWidget(a); box.addWidget(v); box.addWidget(m); sv.addLayout(box); sv.addStretch(1)
        root.addWidget(stats)

        modebar=QFrame(); modebar.setObjectName("ModeBar"); mv=QHBoxLayout(modebar); mv.setContentsMargins(7,7,7,7); mv.setSpacing(6)
        self.mode_buttons=[]
        ...
        self.info=QFrame(); self.info.setObjectName("GenerationStatus"); ih=QHBoxLayout(self.info); ih.setContentsMargins(14,10,14,10); self.status=QLabel("READY · selecciona una operación"); self.status.setObjectName("Hint"); ih.addWidget(self.status); ih.addStretch(1); self.cancel=QPushButton("CANCELAR"); self.cancel.setProperty("class","Danger"); self.cancel.setEnabled(False); self.cancel.clicked.connect(self.request_cancel.emit); ih.addWidget(self.cancel); root.addWidget(self.info)
        self._mode(0)
    ...
    def _video_panel(self):
        ...
        h=QLabel("GENERAR VÍDEO"); h.setObjectName("PanelTitle"); root.addWidget(h); d=QLabel("Render de prototipo usando el launcher canónico de cada familia."); d.setObjectName("PanelMeta"); root.addWidget(d)
        form=QFormLayout(); form.setHorizontalSpacing(24); form.setVerticalSpacing(11)
        self.video_family=self._family_combo(); self.video_seed=QSpinBox(); self.video_seed.setRange(MIN_SEED,MAX_SEED); self.video_seed.setValue(271828); self.video_audio=QCheckBox("Audio"); self.video_audio.setChecked(True); self.video_footer=QCheckBox("Footer editorial"); self.video_footer.setChecked(True)
    ...
    def _review_panel(self):
        ...
        form=QFormLayout(); form.setHorizontalSpacing(24); form.setVerticalSpacing(11)
        self.review_seed_mode=QComboBox(); self.review_seed_mode.addItems(["RANDOM","MANUAL"]); self.review_manual=QLineEdit(); self.review_manual.setPlaceholderText("Cinco seeds únicas, separadas por comas"); self.review_reset=QCheckBox("Recrear assets de review antes de ejecutar"); self.review_reset.setChecked(True)
    ...
    def _production_panel(self):
        ...
        single=QFrame(); single.setObjectName("GeneratePanel"); sv=QVBoxLayout(single); sv.setContentsMargins(18,18,18,18); sv.setSpacing(10); t=QLabel("PRODUCTO ÚNICO"); t.setObjectName("PanelTitle"); sv.addWidget(t); meta=QLabel("Producto final protegido y trazable."); meta.setObjectName("PanelMeta"); sv.addWidget(meta)
        form=QFormLayout(); form.setHorizontalSpacing(18); form.setVerticalSpacing(9); self.prod_family=self._family_combo(); self.prod_seed=QSpinBox(); self.prod_seed.setRange(MIN_SEED,MAX_SEED); self.prod_seed.setValue(271828); self.prod_grammar=QComboBox(); self.prod_audio=QCheckBox("Audio"); self.prod_audio.setChecked(True); self.prod_footer=QCheckBox("Footer editorial"); self.prod_footer.setChecked(True); self.prod_force=QCheckBox("Forzar reemplazo")
        ...
        bulk=QFrame(); bulk.setObjectName("GeneratePanel"); bv=QVBoxLayout(bulk); bv.setContentsMargins(18,18,18,18); bv.setSpacing(10); t=QLabel("PRODUCCIÓN 5×5"); t.setObjectName("PanelTitle"); bv.addWidget(t); meta=QLabel("Cinco seeds compartidas por las cinco familias."); meta.setObjectName("PanelMeta"); bv.addWidget(meta)
        bf=QFormLayout(); bf.setHorizontalSpacing(18); bf.setVerticalSpacing(9); self.prod25_mode=QComboBox(); self.prod25_mode.addItems(["RANDOM","MANUAL"]); self.prod25_manual=QLineEdit(); self.prod25_manual.setPlaceholderText("Cinco seeds únicas"); self.prod25_audio=QCheckBox("Audio"); self.prod25_audio.setChecked(True); self.prod25_footer=QCheckBox("Footer editorial"); self.prod25_footer.setChecked(True); self.prod25_force=QCheckBox("Forzar reemplazo")
    ...
    def _refresh_grammar(self):
        self.prod_grammar.clear(); self.prod_grammar.addItem("AUTO / backend derived",""); f=self.family_by_folder.get(self.prod_family.currentData());
