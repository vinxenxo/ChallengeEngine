...
foreach ($item in $plan) {
    ...
    if (Test-Path -LiteralPath $runDir -PathType Container) {
        ...
        if (Test-Path -LiteralPath $runDir -PathType Container) {
            throw "C11A.1: run directory already exists; refusing overwrite: $runDir. Use a fresh qa/c11a1_challenge_qa or -Resume to retry incomplete runs."
        }
    }
...
Write-Host "[C11A1] Next command: .\tools\finalize_c11a1_challenge_bulk_qa.ps1"
