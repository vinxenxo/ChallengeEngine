...
$Mp4 = Join-Path $ArtifactRoot "InvisibleForces_v1_seed_${Seed}.mp4"
$Gif = Join-Path $ArtifactRoot "InvisibleForces_v1_seed_${Seed}.gif"
$Probe = Join-Path $ArtifactRoot "InvisibleForces_v1_seed_${Seed}_ffprobe.json"
$Audio = Join-Path $ArtifactRoot "InvisibleForces_v1_seed_${Seed}_music.wav"
$AudioScript = Join-Path $PSScriptRoot 'generate_c11c_invisible_forces_v1_music.py'
$GodotLog = Join-Path $ArtifactRoot "InvisibleForces_v1_seed_${Seed}_godot.log"
$Manifest = Join-Path $ArtifactRoot "InvisibleForces_v1_seed_${Seed}_manifest.json"
...
try {
    & godot --path . --scene tools/prototypes/c11c_invisible_forces_v1/InvisibleForcesPrototype.tscn --write-movie $Avi --fixed-fps 30 --quit-after 300 2>&1 | Tee-Object -FilePath $GodotLog
    ...
    $manifestObject = [ordered]@{
        ...
        seed = $Seed
        direction = 'solar_wind_gravitational_topography'
    ...
    Write-Host '[C11-C.5_INVISIBLE_FORCES_V1] PHYSICAL PROTOTYPE PASS - 540x960 / 30 FPS / 300 frames / ~10.0 s + AUDIO'
    Write-Host "[C11-C.5_INVISIBLE_FORCES_V1] MP4: $Mp4"
    Write-Host "[C11-C.5_INVISIBLE_FORCES_V1] GIF: $Gif"
