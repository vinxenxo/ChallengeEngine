...
func _add_footer(root: Control) -> void:
    ...
    var line2: String = "SEED %d | BODY 540X672 | T=18.00S | %d FLOW TRACES | PULSE %.2f" % [_seed, int(_variation["trace_count"]), float(_variation["pulse_speed"])]
    var line3: String = "PALETTE %s | LOOP x%d | AUDIO AMBIENT" % [str(_palette["name"]).to_upper(), int(_variation["loop_cycles"])]
    var line4: String = "DETERMINISTIC FIELD ART / v2.0.4"
