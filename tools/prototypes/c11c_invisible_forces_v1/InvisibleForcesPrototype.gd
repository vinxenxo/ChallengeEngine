...
func _ready() -> void:
    ...
    _palette = PaletteBankClass.palette("invisible_forces", int(_variation["palette_mode"]), _seed)
...
func _add_header(root: Control) -> void:
    ...
    var line2: String = TechnobabbleGeneratorClass.generate_geek_text("invisible_forces", _seed, _variation).to_upper()
...
func _add_footer(root: Control) -> void:
    ...
    var line4: String = "DETERMINISTIC FIELD ART / v2.1.4"
