...
func _build_scene() -> void:
    ...
    _renderer.set_palette(Color(str(_palette["deep"])), Color(str(_palette["primary"])), Color(str(_palette["secondary"])), Color(str(_palette["highlight"])))
...
func _add_header(root: Control) -> void:
    ...
    _header_animator = C11CEditorialAnimatorClass.new(hook, "VISUAL LOOP / INVISIBLE FORCES", _seed)
...
func _add_footer(root: Control) -> void:
    ...
    var line1: String = "VISUAL LOOP // INVISIBLE FORCES"
    var line2: String = geek_text
    var line3: String = "SEED %d | BODY 540X672 | T=18.00S | %d FLOW TRACES | PULSE %.2f" % [_seed, int(_variation["trace_count"]), float(_variation["pulse_speed"])]
    var line4: String = "PALETTE %s | LOOP x%d | AUDIO AMBIENT" % [str(_palette["name"]).to_upper(), int(_variation["loop_cycles"])]
    var line5: String = "DETERMINISTIC FIELD ART / v2.0.1"
