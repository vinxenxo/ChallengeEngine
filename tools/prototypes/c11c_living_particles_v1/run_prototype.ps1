...
$Stem = "LivingParticles_v1_seed_${Seed}$OutputTagSafe"
...
try {
    ...
    $godotArgs=@('--path','.','--scene','tools/prototypes/c11c_living_particles_v1/LivingParticlesPrototype.tscn','--write-movie',$Avi,'--fixed-fps','30','--resolution',$DeliveryResolution,'--quit-after','900')
    ...
    if (-not $NoSound) {
        & python $AudioScript $Audio $Seed $loopCycles $DurationSeconds 'living_particles' 'loop' $audioGrammar
