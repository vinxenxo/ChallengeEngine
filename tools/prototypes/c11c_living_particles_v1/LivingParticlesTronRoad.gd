...
## C11-C 2.2.1 — Living Particles Tron road.
...
func set_contrast_reference(particle_reference: Color, _particle_highlight: Color) -> void:
    ...
    # Pull the complementary hue slightly toward electric blue so the result
    # remains recognisably Tron-like while preserving contrast against particles.
...
func _draw() -> void:
    ...
    var surface_color := Color(_secondary.r, _secondary.g, _secondary.b, 0.12)
    ...
    for lane in rail_positions:
        ...
        var rail_color := Color(_primary.r, _primary.g, _primary.b, 0.42)
    ...
    for lane in lane_positions:
        ...
        var lane_color := Color(_primary.r, _primary.g, _primary.b, 0.23 if absf(lane) > 0.01 else 0.17)
    ...
    for i in range(GRID_ROWS):
        ...
        var alpha := lerpf(0.16, 0.46, perspective)
        var width := 1.0
        if i % 4 == 0:
            alpha += 0.12
            width = 1.4
        var c := Color(_primary.r, _primary.g, _primary.b, clampf(alpha, 0.0, 0.64))
    ...
    for i in range(10):
        ...
        var dash_alpha := lerpf(0.12, 0.32, dash_u)
