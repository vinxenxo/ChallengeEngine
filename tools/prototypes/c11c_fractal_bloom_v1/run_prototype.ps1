...
if([string]::IsNullOrWhiteSpace($OutputRoot)){ $ArtifactRoot = Join-Path $ProjectRoot 'artifacts\prototypes\c11c_fractal_bloom_v1' } else { $ArtifactRoot = [System.IO.Path]::GetFullPath($OutputRoot) }
$OutputTagSafe = if ([string]::IsNullOrWhiteSpace($OutputTag)) { "" } else { "_" + ($OutputTag -replace '[^A-Za-z0-9_-]', '_') }
$Stem = "FractalBloom_v1_seed_${Seed}$OutputTagSafe"
...
try {
    ...
    $godotArgs=@('--path','.','--scene','tools/prototypes/c11c_fractal_bloom_v1/FractalBloomPrototype.tscn','--write-movie',$Avi,'--fixed-fps','30','--resolution',$DeliveryResolution,'--quit-after','900')
    ...
    if(-not (Test-Path -LiteralPath $Authoring)){
        $canonicalAuthoring=Join-Path $ProjectRoot ('artifacts\prototypes\c11c_fractal_bloom_v1\' + ($Stem + '_authoring.json'))
