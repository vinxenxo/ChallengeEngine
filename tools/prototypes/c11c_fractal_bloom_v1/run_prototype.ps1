...
$Mp4 = Join-Path $ArtifactRoot "FractalBloom_v1_seed_${Seed}.mp4"
$Gif = Join-Path $ArtifactRoot "FractalBloom_v1_seed_${Seed}.gif"
$Probe = Join-Path $ArtifactRoot "FractalBloom_v1_seed_${Seed}_ffprobe.json"
$Audio = Join-Path $ArtifactRoot "FractalBloom_v1_seed_${Seed}_music.wav"
$AudioScript = Join-Path $PSScriptRoot 'generate_fractal_bloom_music.py'
$GodotLog = Join-Path $ArtifactRoot "FractalBloom_v1_seed_${Seed}_godot.log"
$Manifest = Join-Path $ArtifactRoot "FractalBloom_v1_seed_${Seed}_manifest.json"
...
try {
    & godot --path . --scene tools/prototypes/c11c_fractal_bloom_v1/FractalBloomPrototype.tscn `
        --write-movie $Avi `
        --fixed-fps 30 `
    ...
    if ((Get-Item -LiteralPath $Audio).Length -le 0) { throw 'Music WAV is empty.' }

    & ffmpeg -y -hide_banner -loglevel error `
        -i $Avi -an -c:v libx264 -preset fast -crf 18 -pix_fmt yuv420p -movflags +faststart `
        $Mp4Silent
    ...
    $manifestObject = [ordered]@{
        ...
        seed = $Seed
        direction = 'radial_biological_fractal'
        visual = [ordered]@{
            canvas = '540x960'
            body = 'y=144..816'
            ...
            fps = 30
            frame_count = 300
            mathematical_model = 'dynamic Julia + orbit traps + domain warp + analytical periodic zoom'
            depth_layers = 3
        }
        editorial = [ordered]@{
            header = 'ENTRA EN UN UNIVERSO INFINITO'
            footer = 'ZN+1 = ZN^2 + C | ORBIT TRAP | JULIA | 24 ITER'
        }
        audio = [ordered]@{
            mode = 'prototype_procedural_music'
            deterministic = $true
            source_script = 'generate_fractal_bloom_music.py'
            sample_rate = 44100
            channels = 2
            duration_seconds = 10.0
            muxed_into_mp4 = $true
    ...
    Write-Host '[C11-C.2] PHYSICAL PROTOTYPE PASS - 540x960 / 30 FPS / 300 frames / ~10.0 s + AUDIO'
    Write-Host "[C11-C.2] MP4: $Mp4"
    Write-Host "[C11-C.2] GIF: $Gif"
    Write-Host "[C11-C.2] AUDIO: $Audio"
} finally {
    Pop-Location
}
