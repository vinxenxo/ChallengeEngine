...
$ArtifactRoot = Join-Path $ProjectRoot 'artifacts\prototypes\c11c_sacred_symmetry_v1'
$Avi = Join-Path $ArtifactRoot "SacredSymmetry_v1_seed_${Seed}.avi"
$Mp4Silent = Join-Path $ArtifactRoot "SacredSymmetry_v1_seed_${Seed}_silent.mp4"
$Mp4 = Join-Path $ArtifactRoot "SacredSymmetry_v1_seed_${Seed}.mp4"
$Gif = Join-Path $ArtifactRoot "SacredSymmetry_v1_seed_${Seed}.gif"
$Probe = Join-Path $ArtifactRoot "SacredSymmetry_v1_seed_${Seed}_ffprobe.json"
$Audio = Join-Path $ArtifactRoot "SacredSymmetry_v1_seed_${Seed}_music.wav"
$AudioScript = Join-Path $PSScriptRoot 'generate_c11c_sacred_symmetry_v1_music.py'
$GodotLog = Join-Path $ArtifactRoot "SacredSymmetry_v1_seed_${Seed}_godot.log"
$Manifest = Join-Path $ArtifactRoot "SacredSymmetry_v1_seed_${Seed}_manifest.json"
...
try {
    & godot --path . --scene tools/prototypes/c11c_sacred_symmetry_v1/SacredSymmetryPrototype.tscn --write-movie $Avi --fixed-fps 30 --quit-after 300 2>&1 | Tee-Object -FilePath $GodotLog
    ...
    $manifestObject = [ordered]@{
        ...
        seed = $Seed
        direction = 'generative_astrolabe_celestial_clockwork'
        visual = [ordered]@{ canvas='540x960'; body='y=144..816'; duration_seconds=10.0; fps=30; frame_count=300; mathematical_model='radial symmetry + concentric rings + polar ticks + orbital nodes + periodic clockwork motion'; }
        editorial = [ordered]@{ header='EL MECANISMO QUE ORDENA EL CAOS'; footer='RADIAL SYMMETRY  |  N=12  |  POLAR ASTROLABE  |  3 RINGS'; }
        audio = [ordered]@{ mode='prototype_procedural_music'; deterministic=$true; source_script='generate_c11c_sacred_symmetry_v1_music.py'; sample_rate=44100; channels=2; duration_seconds=10.0; muxed_into_mp4=$true; C7_modified=$false; }
    ...
    Write-Host '[C11-C.3_SACRED_SYMMETRY_V1] PHYSICAL PROTOTYPE PASS - 540x960 / 30 FPS / 300 frames / ~10.0 s + AUDIO'
    Write-Host "[C11-C.3_SACRED_SYMMETRY_V1] MP4: $Mp4"
    Write-Host "[C11-C.3_SACRED_SYMMETRY_V1] GIF: $Gif"
