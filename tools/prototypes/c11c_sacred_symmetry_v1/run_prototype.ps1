...
if([string]::IsNullOrWhiteSpace($OutputRoot)){ $ArtifactRoot = Join-Path $ProjectRoot 'artifacts\prototypes\c11c_sacred_symmetry_v1' } else { $ArtifactRoot = [System.IO.Path]::GetFullPath($OutputRoot) }
$OutputTagSafe = if ([string]::IsNullOrWhiteSpace($OutputTag)) { "" } else { "_" + ($OutputTag -replace '[^A-Za-z0-9_-]', '_') }
$Stem = "SacredSymmetry_v1_seed_${Seed}$OutputTagSafe"
...
try {
    ...
    $godotArgs=@('--path','.','--scene','tools/prototypes/c11c_sacred_symmetry_v1/SacredSymmetryPrototype.tscn','--write-movie',$Avi,'--fixed-fps','30','--resolution',$DeliveryResolution,'--quit-after','900')
    ...
    if(-not (Test-Path -LiteralPath $Authoring)){
        $canonicalAuthoring=Join-Path $ProjectRoot ('artifacts\prototypes\c11c_sacred_symmetry_v1\' + ($Stem + '_authoring.json'))
    ...
    if (-not $NoSound) {
        & python $AudioScript $Audio $Seed $loopCycles $DurationSeconds 'sacred_symmetry' 'loop' $audioGrammar
    ...
    $repro = '.\tools\prototypes\c11c_sacred_symmetry_v1\run_prototype.ps1 -Seed ' + [string]$Seed
    ...
    $manifestObject = [ordered]@{
        ...
        technical_id = 'kaleidoscope'
        artistic_name = 'Sacred Symmetry'
        production_id = 'c11c_sacred_symmetry_v1'
        runtime_id = 'sacred_symmetry'
