...
func _build_scene() -> void:
    ...
    _renderer.set_palette(Color(str(_palette["primary"])), Color(str(_palette["secondary"])), Color(str(_palette["highlight"])), Color(str(_palette["white_gold"])), Color(str(_palette["accent"])))
    _renderer.set_style(
...
func _add_header(root: Control) -> void:
    ...
    _header_animator = C11CEditorialAnimatorClass.new(hook, "VISUAL LOOP / SACRED SYMMETRY", _seed)
...
func _add_footer(root: Control) -> void:
    ...
    var line1: String = "VISUAL LOOP // SACRED SYMMETRY"
    var line2: String = geek_text
    var line3: String = "SEED %d | BODY 540X672 | T=18.00S | N=%d | GEAR %d:%d" % [_seed, int(_variation["symmetry_order"]), int(_variation["gear_inner"]), int(_variation["gear_outer"])]
    var line4: String = "PALETTE %s | LOOP x%d | AUDIO AMBIENT" % [str(_palette["name"]).to_upper(), int(_variation["loop_cycles"])]
    var line5: String = "PRECISION CELESTIAL MECHANISM / v2.0.1"
