...
if($bulk -notmatch '\$launcherParams=@\{Seed=\[int\]\$seed\}'){throw 'Multiseed must invoke family launchers through typed splatting.'}
$review=Get-Content -Raw (Join-Path $PSScriptRoot 'run_c11c_art_direction_review.ps1')
if($review -notmatch '\$bulkParams=@\{Seeds=@\(\$Seeds\)\}'){throw 'Art-direction review must pass seed arrays through typed splatting.'}
$exporter=Get-Content -Raw (Join-Path $PSScriptRoot 'export_all_review_assets.ps1')
if($exporter -notmatch '\$gifParams=@\{InputRoot=\$InputRoot;OutputRoot=\$OutputRoot;Seeds=@\(\$Seeds\)\}'){throw 'Review GIF exporter must use typed seed splatting.'}
...
Write-Host '[C11C-DELIVERY-VALIDATION] PASS - Godot 720x1280, typed PowerShell arrays/switches, no nested PowerShell, review/production separation.'
return
