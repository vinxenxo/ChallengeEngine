...
foreach ($family in $families) {
    ...
    foreach ($seed in $Seeds) {
        ...
        $exit = $LASTEXITCODE
        if ($exit -ne 0) { throw "Multi-seed run failed for ${family} seed=${seed}: exit=$exit" }
        ...
        foreach ($required in @($mp4,$probePath,$social)) {
            if (-not (Test-Path -LiteralPath $required)) { throw "Missing artifact after successful launcher: $required" }
        }
        ...
        if ([int]$video.nb_frames -ne 540) { throw "Frame count contract failed for ${family} seed=${seed}: $($video.nb_frames)" }
        if ([math]::Abs([double]$video.duration - 18.0) -gt 0.05) { throw "Duration contract failed for ${family} seed=${seed}: $($video.duration)" }
        $results += [ordered]@{ family=$family; seed=$seed; status='PASS'; mp4=$mp4 }
        Write-Host "[C11-C-MULTISEED] PASS family=$family seed=$seed 720x1280 / 30 FPS / 540 frames / 18.0s"
    }
}

$manifest = [ordered]@{
    schema='C11-C-MULTISEED-BULK-V2'; revision='2.1.2'; status='COMPLETE'
    seeds=@($Seeds); family_count=$families.Count; seed_count=$Seeds.Count; render_count=$families.Count*$Seeds.Count
    results=$results
    visual_contract=[ordered]@{canvas='720x1280'; body='y=192..1088'; fps=30; duration_seconds=18.0; frames=540}
    audio_contract='default ON; safe mobile ambient; -NoSound/-Silent disables audio'
