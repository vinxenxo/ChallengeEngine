...
foreach($family in $families){
    ...
    foreach($seed in $Seeds){
        ...
        if($duration -lt 20.0 -or $duration -gt 23.0){throw "Duration contract failed for ${family} seed=${seed}: $duration (expected 20..23s)"}
...
$manifest=[ordered]@{schema='C11-C-MULTISEED-BULK-V2';revision='2.14.0';status='COMPLETE';seeds=@($Seeds);family_count=$families.Count;seed_count=$Seeds.Count;render_count=$families.Count*$Seeds.Count;results=$results;visual_contract=[ordered]@{canvas='720x1280';body='y=192..1088';fps=30;duration_seconds='20..23 policy-driven';frames='round(duration_seconds*30)'};audio_contract='default ON; safe mobile ambient; -NoSound/-Silent disables audio';frozen_boundaries_modified=$false}
