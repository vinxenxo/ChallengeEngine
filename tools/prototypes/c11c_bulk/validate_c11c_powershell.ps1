...
foreach($family in @('c11c_geometric_waves_v1','c11c_fractal_bloom_v1','c11c_sacred_symmetry_v1','c11c_living_particles_v1','c11c_invisible_forces_v1')){$files += Join-Path $ProjectRoot ("tools\prototypes\$family\run_prototype.ps1")}
$errors=0
foreach($file in $files){
  ...
  if($parseErrors.Count -eq 0){Write-Host "[C11C-PS-PARSE] PASS $file"} else {$errors++;Write-Host "[C11C-PS-PARSE] FAIL $file";$parseErrors|ForEach-Object{Write-Host "  $($_.Message) line=$($_.Extent.StartLineNumber) col=$($_.Extent.StartColumnNumber)"}}
}
