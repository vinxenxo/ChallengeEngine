...
if ($Seeds.Count -eq 0) {
    ...
    throw "When -Seeds is supplied, pass exactly $VariationsPerFamily seeds."
}
...
$bulkArgs = @('-NoProfile','-ExecutionPolicy','Bypass','-File',$Bulk,'-Seeds') + @($Seeds | ForEach-Object { [string]$_ })
& powershell.exe @bulkArgs
$exit = $LASTEXITCODE
if ($exit -ne 0) { throw "Multiseed bulk failed: exit=$exit" }

$reviewArgs = @('-NoProfile','-ExecutionPolicy','Bypass','-File',$Reviews,'-InputRoot',(Join-Path $ProjectRoot 'artifacts\prototypes'),'-OutputRoot',$ReviewRoot,'-Seeds') + @($Seeds | ForEach-Object { [string]$_ })
& powershell.exe @reviewArgs
$exit = $LASTEXITCODE
if ($exit -ne 0) { throw "Review asset export failed: exit=$exit" }

$expectedMp4Count = $VariationsPerFamily * 5
$stageMp4s = @(Get-ChildItem -Path (Join-Path $ProjectRoot 'artifacts\prototypes') -Recurse -Filter '*.mp4' -File | Where-Object { $_.Name -match '_seed_[0-9]+\.mp4$' -and $_.FullName -notlike '*c11c_review_assets*' })
$batchMp4s = @($stageMp4s | Where-Object { ($Seeds | ForEach-Object { $_.ToString() }) -contains ([regex]::Match($_.Name,'_seed_([0-9]+)\.mp4$').Groups[1].Value) })
if ($batchMp4s.Count -ne $expectedMp4Count) { throw "Review corpus count failed: expected $expectedMp4Count canonical MP4s for this seed batch, found $($batchMp4s.Count)." }

$seedManifest = [ordered]@{
    schema='C11-C-ART-DIRECTION-REVIEW-SEEDS-V2'; revision='2.1.2'; mode='random_or_explicit_seed_batch'
    ...
    delivery_contract='720x1280 / 30 FPS / 18.0 s / 540 frames'
}
$seedManifestPath = Join-Path $ReviewRoot 'C11-C_ART_DIRECTION_REVIEW_SEEDS.json'
[System.IO.File]::WriteAllText($seedManifestPath,($seedManifest | ConvertTo-Json -Depth 8),(New-Object System.Text.UTF8Encoding($false)))
