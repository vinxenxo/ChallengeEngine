...
try {
    & godot --path . --scene tools/prototypes/c11c_geometric_waves_v1/GeometricWavesPrototype.tscn --write-movie $Avi --fixed-fps 30 --resolution 720x1280 --quit-after 540 2>&1 | Tee-Object -FilePath $GodotLog
    ...
    if (-not $NoSound) {
        & python $AudioScript $Audio $Seed $loopCycles $DurationSeconds 'geometric_waves'
