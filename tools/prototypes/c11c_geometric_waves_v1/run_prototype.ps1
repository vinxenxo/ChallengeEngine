...
if([string]::IsNullOrWhiteSpace($OutputRoot)){ $ArtifactRoot = Join-Path $ProjectRoot 'artifacts\prototypes\c11c_geometric_waves_v1' } else { $ArtifactRoot = [System.IO.Path]::GetFullPath($OutputRoot) }
$OutputTagSafe = if ([string]::IsNullOrWhiteSpace($OutputTag)) { "" } else { "_" + ($OutputTag -replace '[^A-Za-z0-9_-]', '_') }
$Stem = "GeometricWaves_v1_seed_${Seed}$OutputTagSafe"
...
try {
    ...
    if(-not (Test-Path -LiteralPath $Authoring)){
        $canonicalAuthoring=Join-Path $ProjectRoot ('artifacts\prototypes\c11c_geometric_waves_v1\' + ($Stem + '_authoring.json'))
    ...
    if (-not $NoSound) {
        & python $AudioScript $Audio $Seed $loopCycles $DurationSeconds 'geometric' 'loop' $audioGrammar
    ...
    $repro = '.\tools\prototypes\c11c_geometric_waves_v1\run_prototype.ps1 -Seed ' + [string]$Seed
    ...
    $manifestObject = [ordered]@{
        ...
        technical_id = 'geometric'
        artistic_name = 'Geometric Waves'
        production_id = 'c11c_geometric_waves_v1'
        runtime_id = 'geometric'
