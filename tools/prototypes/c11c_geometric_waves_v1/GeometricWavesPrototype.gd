...
func _build_scene() -> void:
    ...
    _renderer.set_palette(Color(str(_palette["dominant"])), Color(str(_palette["secondary"])), Color("FFFFFF"))
    _renderer.set_style(
...
func _add_header(root: Control) -> void:
    ...
    var hooks: Array[String] = ["CUANDO UNA ONDA SE CONVIERTE EN ARQUITECTURA", "OBSERVA COMO VIAJA LA FORMA", "LA GEOMETRIA TAMBIEN PUEDE FLUIR", "ESCULTURA HECHA DE ONDAS", "ENTRA EN EL CAMPO ARMONICO"]
    ...
    line1: String = "%s | WAVE %.1f | MORPH %.2f" % [grammar_name.replace(" ", "_"), float(_variation["wave_frequency"]), float(_variation["morph"])]
    ...
    _header_animator = C11CEditorialAnimatorClass.new(hook, "VISUAL LOOP / GEOMETRIC WAVES", _seed)

func _add_footer(root: Control) -> void:
    var geek_text: String = TechnobabbleGeneratorClass.generate_geek_text("geometric", _seed, _variation)
    var line1: String = "VISUAL LOOP // GEOMETRIC WAVES"
    var line2: String = geek_text
    var line3: String = "SEED %d | BODY 540X672 | T=18.00S | %d FPS | %d LOOPS" % [_seed, FPS, int(_variation["loop_cycles"])]
    var line4: String = "PALETTE %s | LOOP x%d | AUDIO AMBIENT" % [str(_palette["name"]).to_upper(), int(_variation["loop_cycles"])]
    var line5: String = "GEOMETRIC GENERATIVE WAVE / v2.0.1"
