...
func _add_footer(root: Control) -> void:
    ...
    var hook := _new_label("CUANDO LAS ONDAS DIBUJAN GEOMETRÍA", Vector2(36.0, 53.0), Vector2(468.0, 38.0), 22, Color("F3F7FF"))
    hook.add_theme_constant_override("outline_size", 2)
    root.add_child(hook)
    root.add_child(_new_label("MATHEMATICAL GENERATIVE ART  /  v1", Vector2(36.0, 101.0), Vector2(468.0, 18.0), 9, Color("587086")))
