# res://tests/C6F4CanonicalV2HitTest.gd
extends SceneTree
...
func _initialize() -> void:
	print("[TEST] Running C6F4CanonicalV2HitTest...")
	_run_test()

	if failures.is_empty():
		print("[C6F4_CANONICAL_V2_HIT_SUITE] PASS")
		quit(0)
		return

	for failure in failures:
		printerr("FAIL: " + failure)
	print("[C6F4_CANONICAL_V2_HIT_SUITE] FAIL count=%d" % failures.size())
	quit(1)

func _run_test() -> void:
	var path := "res://challenges/CHALLENGE_005.json"
	var legacy = JSON.parse_string(FileAccess.get_file_as_string(path))
	if not legacy is Dictionary or legacy.is_empty():
		failures.append("Unable to read CHALLENGE_005.json")
		return
	...
	if not params.has("hit"):
		failures.append("Canonical V2 simulation.parameters missing hit block.")
	...
	# Effective runtime must use pure V2 for hit_v1.
	...
	if runtime_input.has("generation"):
		failures.append("Effective hit_v1 runtime input still contains legacy generation.")
	if not (runtime_input.get("video", null) is Dictionary):
		failures.append("Effective hit_v1 runtime input does not preserve inline Canonical V2 video.")
	var root_difficulty: Dictionary = runtime_input.get("difficulty", {})
	if root_difficulty.has("hit"):
		failures.append("Effective hit_v1 runtime input restored legacy mechanical difficulty fields.")
