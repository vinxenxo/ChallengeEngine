# res://tests/C6F4CanonicalV2CatchTest.gd
extends SceneTree
...
func _initialize() -> void:
	print("[TEST] Running C6F4CanonicalV2CatchTest...")
	_run_test()

	if failures.is_empty():
		print("[C6F4_CANONICAL_V2_CATCH_SUITE] PASS")
		quit(0)
		return

	for failure in failures:
		printerr("FAIL: " + failure)
	print("[C6F4_CANONICAL_V2_CATCH_SUITE] FAIL count=%d" % failures.size())
	quit(1)

func _run_test() -> void:
	var path := "res://challenges/CHALLENGE_006.json"
	var legacy = JSON.parse_string(FileAccess.get_file_as_string(path))
	if not legacy is Dictionary or legacy.is_empty():
		failures.append("Unable to read CHALLENGE_006.json")
		return
	...
	var params = canonical.get("simulation", {}).get("parameters", {})
	if not params.has("catch"):
		failures.append("Canonical V2 simulation.parameters missing catch block.")
	else:
		var catch_params = params.get("catch", {})
		if not catch_params is Dictionary or catch_params.is_empty():
			failures.append("Canonical V2 simulation.parameters.catch is not a non-empty Dictionary.")
	...
	# Effective runtime must use pure V2 for catch_v1.
	...
	if runtime_input.has("generation"):
		failures.append("Effective catch_v1 runtime input still contains legacy generation.")
	if not (runtime_input.get("video", null) is Dictionary):
		failures.append("Effective catch_v1 runtime input does not preserve inline Canonical V2 video.")
	var root_difficulty: Dictionary = runtime_input.get("difficulty", {})
	if root_difficulty.has("catch"):
		failures.append("Effective catch_v1 runtime input restored legacy mechanical difficulty fields.")
