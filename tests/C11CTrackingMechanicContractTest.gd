...
func _initialize() -> void:
	...
	_test_final_history_is_full(generator, params)
...
func _test_state_shape(generator: TrackingGeneratorClass, params: Dictionary) -> void:
	var state := generator.generate_with_variation(20, 630, params, {"tracking_variant": 0.25})
	...
	_assert(is_equal_approx(float(state.get("trajectory_state", {}).get("travel_cycles", 0.0)), 0.75), "Tracking gameplay baseline must use 0.75 travel cycles.")
...
func _test_target_motion(generator: TrackingGeneratorClass, params: Dictionary) -> void:
	var state_a := generator.generate(0, 630, params)
	var state_b := generator.generate(315, 630, params)
	...
	for frame in range(0, 629):
		var state0 := generator.generate(frame, 630, params)
		var state1 := generator.generate(frame + 1, 630, params)
	...
	for frame in range(630):
		var state := generator.generate(frame, 630, params)
		...
		_assert(speed <= 220.0, "Tracking baseline speed is too high for the intended smooth-pursuit presentation.")
...
func _test_history_only_trail(generator: TrackingGeneratorClass, params: Dictionary) -> void:
	var state := generator.generate(30, 630, params)
	var trail: Array = state.get("trajectory_state", {}).get("trail_points", [])
	_assert(trail.size() == 31, "Tracking growing history must contain frames 0..30 at gameplay frame 30.")
...
func _test_final_history_is_full(generator: TrackingGeneratorClass, params: Dictionary) -> void:
	var state: Dictionary = generator.generate(629, 630, params)
	var trajectory: Dictionary = state.get("trajectory_state", {})
	var trail: Array = trajectory.get("trail_points", [])
	_assert(str(trajectory.get("trail_behavior", "")) == "growing_history", "Tracking trail must use growing_history behavior.")
	_assert(trail.size() == 630, "Tracking growing history must reach the full gameplay history at the final frame.")

func _test_cosmetic_variant_does_not_change_mechanics(generator: TrackingGeneratorClass, params: Dictionary) -> void:
	var base := generator.generate_with_variation(24, 630, params, {"tracking_variant": 0.0})
	var variant := generator.generate_with_variation(24, 630, params, {"tracking_variant": 0.99})
...
func _test_determinism(generator: TrackingGeneratorClass, params: Dictionary) -> void:
	var state_a := generator.generate(37, 630, params)
	var state_b := generator.generate(37, 630, params)
...
func _params() -> Dictionary:
	return {
		...
		"distractors": [],
		"trajectory": {"type": "lissajous", "travel_cycles": 0.75, "x_frequency": 2.0, "y_frequency": 3.0, "amplitude_x": 160.0, "amplitude_y": 210.0, "center_x": 270.0, "center_y": 480.0, "phase_x": 0.25, "phase_y": -0.65, "target_radius": 12.0, "trail_length_frames": 630, "trail_mode": "growing_history"},
