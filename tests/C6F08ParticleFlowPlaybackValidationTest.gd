# res://tests/C6F08ParticleFlowPlaybackValidationTest.gd
extends SceneTree

# ============================================================
# C6-F0.8-D5-C — Particle Flow Playback & Physical Validation
...
func _initialize() -> void:
	print("[TEST] Running C6F08ParticleFlowPlaybackValidationTest...")
	await _run_tests()
...
func _run_tests() -> void:
	var def_a = _create_mock_definition(12345, "particle_flow")
	var def_b = _create_mock_definition(99999, "particle_flow")
	var def_legacy = _create_mock_definition(12345, "kaleidoscope")
	
	var path_a = "user://c6f08_pf_mock_a.json"
	var path_b = "user://c6f08_pf_mock_b.json"
	var path_legacy = "user://c6f08_pf_mock_legacy.json"
	
	_write_json(path_a, def_a)
	_write_json(path_b, def_b)
	_write_json(path_legacy, def_legacy)
	
	var state_a = await _run_player_and_validate(path_a, "PARTICLE_A")
	var state_b = await _run_player_and_validate(path_b, "PARTICLE_B")
	
	if state_a != null and state_b != null:
		_assert(str(state_a) != str(state_b), "Fallo D5: Diferentes semillas deben variar salida de partículas.")
...
func _run_player_and_validate(def_path: String, mode: String):
	...
	while not player.playback_finished:
		if player._current_frame_index == 1 and captured_params == null: 
			var pf_renderer = _find_renderer(player, "ParticleFlowRenderer.gd")
			
			if mode.begins_with("PARTICLE"):
				if pf_renderer == null:
					_assert(false, "[%s] ParticleFlowRenderer must exist." % mode)
				else:
					_assert(pf_renderer.visible, "[%s] ParticleFlowRenderer must be visible." % mode)
					var rect = pf_renderer.get("_rect")
					if rect == null:
				...
				if pf_renderer != null:
					_assert(not pf_renderer.visible, "[%s] Legacy Protection: Renderer MUST be hidden." % mode)
...
func _conclude() -> void:
	if failures.is_empty():
		print("[C6F08_PARTICLE_FLOW_PLAYBACK_VALIDATION_SUITE] PASS")
		quit(0)
	else:
		for failure in failures:
			push_error(failure)
		print("[C6F08_PARTICLE_FLOW_PLAYBACK_VALIDATION_SUITE] FAIL failures=%d" % failures.size())
		quit(1)