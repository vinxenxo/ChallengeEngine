# res://tests/C9FCatchAuthoringProductiveTest.gd
extends SceneTree
...
func _initialize() -> void:
	...
	var request_result := ChallengeAuthoringRequest.create_from_dictionary({
		...
		"level": DEFAULT_LEVEL,
		"seed": DEFAULT_SEED,
		"profile": "default_catch_v1",
		"overrides": {},
		"content": {
			"hook": "¡ATRAPA EL OBJETIVO!",
			"reveal": "",
	...
	if canonical.get("mechanic") != "catch_v1" or canonical.get("mechanic_version") != "1.0":
		printerr("FAIL: mechanic identity mismatch.")
		quit(1)
		return
	if canonical.get("asset_family") != "fam_catch_01":
	...
	if float(params.get("catch_radius", -1.0)) != 45.0:
		printerr("FAIL: catch_radius must match CHALLENGE_006 oracle contract.")
		quit(1)
		return
	if float(params.get("target_speed_base", -1.0)) != 2.5:
		printerr("FAIL: target_speed_base must match CHALLENGE_006 oracle contract.")
		quit(1)
		return
	if float(params.get("catcher_speed_base", -1.0)) != 6.0:
		printerr("FAIL: catcher_speed_base must match CHALLENGE_006 oracle contract.")
		quit(1)
		return
	if params.get("catcher_origin") != [540.0, 1700.0] or params.get("catcher_direction") != [0.0, -1.0]:
		printerr("FAIL: catcher origin/direction must match CHALLENGE_006 oracle contract.")
		quit(1)
		return
	if params.get("target_origin") != [540.0, 400.0] or params.get("target_direction") != [0.0, 1.0]:
		printerr("FAIL: target origin/direction must match CHALLENGE_006 oracle contract.")
		quit(1)
		return

	if canonical.get("video", {}).get("profile_id") != "f4_legacy_60_h0_g7_r0_c2":
		printerr("FAIL: video profile identity mismatch.")
		quit(1)
		return
	var audio: Dictionary = canonical.get("audio", {})
	if audio.get("enabled") != true or audio.get("profile_id") != "c7_profile_mixed_alt":
	...
	if runtime_projection.is_empty():
		...
		quit(1)
		return
	if runtime_projection.get("video_profile") != "f4_legacy_60_h0_g7_r0_c2":
	...
	print("[C9F_CATCH_V1_PRODUCTIVE_GENERATION] PASS")
	print("[C9F_CATCH_V1_PRODUCTIVE_GENERATION] GENERATED_CONFIG=%s" % output_path)
	quit(0)
